...
use Enalquiler\Elastica\Bulk\Action\AbstractDocument;
use Enalquiler\Elastica\Document;
use Enalquiler\Elastica\Exception\Bulk\ResponseException;
...
class BulkTest extends BaseTest
{
    ...
    public function testSend()
    {
        ...
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action\IndexDocument', $actions[0]);
        $this->assertEquals('index', $actions[0]->getOpType());
        $this->assertSame($newDocument1, $actions[0]->getDocument());

        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action\IndexDocument', $actions[1]);
        $this->assertEquals('index', $actions[1]->getOpType());
        $this->assertSame($newDocument2, $actions[1]->getDocument());

        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action\CreateDocument', $actions[2]);
        $this->assertEquals('create', $actions[2]->getOpType());
        $this->assertSame($newDocument3, $actions[2]->getDocument());

        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action\IndexDocument', $actions[3]);
...
';
        ...
        foreach ($response as $i => $bulkResponse) {
            $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Response', $bulkResponse);
    ...
    public function testAddRawData()
    {
        ...
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action', $actions[0]);
        $this->assertEquals('index', $actions[0]->getOpType());
        ...
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action', $actions[1]);
        ...
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action', $actions[2]);
        ...
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action', $actions[3]);
        ...
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk\Action', $actions[4]);
...
';
    ...
    public function testUpdate()
    {
        ...
        $bulk = new Bulk($client);
        $bulk->setType($type);
        $updateAction = new \Enalquiler\Elastica\Bulk\Action\UpdateDocument($doc2);
        ...
        $script = new \Enalquiler\Elastica\Script\Script('ctx._source.name += param1;', ['param1' => ' was Paul'], null, 2);
        $doc2 = new Document();
        ...
        //test upsert
        $script = new \Enalquiler\Elastica\Script\Script('ctx._scource.counter += count', ['count' => 1], null, 5);
        ...
        //test doc_as_upsert
        $doc = new \Enalquiler\Elastica\Document(6, ['test' => 'test']);
        ...
        $doc1 = new \Enalquiler\Elastica\Document(7, ['test' => 'test1']);
        $doc1->setDocAsUpsert(true);
        $doc2 = new \Enalquiler\Elastica\Document(8, ['test' => 'test2']);
        ...
        $updateAction = new \Enalquiler\Elastica\Bulk\Action\UpdateDocument($doc3);
        $bulk->addAction($updateAction);
        $response = $bulk->send();
    ...
    public function testRetry()
    {
        ...
        $script = new \Enalquiler\Elastica\Script\Script('');
    ...
    {
        $bulk = new Bulk($this->_getClient());
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk', $bulk->setShardTimeout(10));
    }
    ...
    {
        $bulk = new Bulk($this->_getClient());
        $this->assertInstanceOf('Enalquiler\Elastica\Bulk', $bulk->setRequestParam('key', 'value'));
    }
